  ...
  return (
    ...
    >
      <div className="bg-white dark:bg-neutral-900 p-2 rounded-[25px] shadow-[0px_10px_20px_0px_#D3D3D3] dark:shadow-none border border-neutral-100 dark:border-neutral-800">
        {icon ? (
          <div className={`w-8 h-8 ${textColor} z-20 mb-6 mx-auto`}>{icon}</div>
        ) : (
        ...
        >
          ...
          {imageUrl && (
            <div className="mt-4 w-full h-[120px] rounded-[10px] overflow-hidden relative">
               <img src={imageUrl} alt={title} className="w-full h-full object-cover transition-transform hover:scale-110 duration-500" />
            </div>
          )}
        </div>
      </div>
